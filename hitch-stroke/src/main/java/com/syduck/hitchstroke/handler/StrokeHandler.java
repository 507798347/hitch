...
import com.syduck.hitchcommons.domain.bo.*;
import com.syduck.hitchcommons.domain.vo.response.ResponseVO;
import com.syduck.hitchcommons.enums.BusinessErrors;
import com.syduck.hitchcommons.enums.InviteState;
import com.syduck.hitchcommons.enums.QuickConfirmState;
...
import com.syduck.hitchcommons.utils.RedisHelper;
...
import com.syduck.hitchmodules.vo.StrokeVO;
import com.syduck.hitchstroke.handler.valuation.BasicValuation;
import com.syduck.hitchstroke.handler.valuation.FuelCostValuation;
import com.syduck.hitchstroke.handler.valuation.StartPriceValuation;
import com.syduck.hitchstroke.handler.valuation.Valuation;
import com.syduck.hitchstroke.rabbitmq.MQProducer;
import com.syduck.hitchstroke.service.AccountAPIService;
import com.syduck.hitchstroke.service.OrderAPIService;
import com.syduck.hitchstroke.service.StrokeAPIService;
...
public class StrokeHandler {
    ...
    public void sendStartGeo(WorldMapBO worldMapBO) {
        kafkaTemplate.send(HitchConstants.STROKE_START_GEO, JSON.toJSONString(worldMapBO));
    }
    ...
    public ResponseVO<StrokeVO> invite(StrokeVO strokeVO) {
        ...
        //创建邀请状态
        // 0 = 未确认， 1 = 已确认 ， 2= 已拒绝
        redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviteeTripId, inviterTripId, String.valueOf(InviteState.UNCONFIRMED.getCode()));
        redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId, inviteeTripId, String.valueOf(InviteState.UNCONFIRMED.getCode()));
        //发送延时消息
    ...
    public void timeoutHandel(StrokeVO strokeVO) {
        ...
        //获取乘客行程ID
        String inviteeTripId = strokeVO.getInviteeTripId();
        String tripeeStatus = redisHelper.getHash(HitchConstants.STROKE_INVITE_PREFIX, inviteeTripId, inviterTripId);
        String triperStatus = redisHelper.getHash(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId, inviteeTripId);
        //如果是邀请状态，未确认
        if (String.valueOf(InviteState.UNCONFIRMED.getCode()).equals(tripeeStatus)
            ...
            //设置为超时状态
            redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviteeTripId, inviterTripId, String.valueOf(InviteState.TIMEOUT.getCode()));
            redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId, inviteeTripId, String.valueOf(InviteState.TIMEOUT.getCode()));
        }
    }
    ...
    public ResponseVO<StrokeVO> departure(String tripid) {
        ...
        redisHelper.delGEO(HitchConstants.STROKE_DRIVER_GEO_START, "", tripid);
        redisHelper.delGEO(HitchConstants.STROKE_DRIVER_GEO_END, "", tripid);

        //获取司机zset里放的乘客
        List<ZsetResultBO> zsetResultBOList = redisHelper.getZsetSortValues(HitchConstants.STROKE_GEO_ZSET_PREFIX, tripid);
        //遍历这些乘客的zset，删除隐藏的司机信息
        for (ZsetResultBO zsetResultBO : zsetResultBOList) {
            //删除乘客zset里隐藏的司机行程信息
            redisHelper.delZsetByKey(HitchConstants.STROKE_GEO_ZSET_PREFIX, zsetResultBO.getValue(), tripid);
        }
        //删除司机的zset
        redisHelper.delKey(HitchConstants.STROKE_GEO_ZSET_PREFIX, tripid);

        //获取司机的邀请列表
        Map<String, String> inviteMap = redisHelper.getHashByMap(HitchConstants.STROKE_INVITE_PREFIX, tripid);
        //删除乘客列表中的司机信息
        for (Map.Entry<String, String> entry : inviteMap.entrySet()) {
            redisHelper.delHash(HitchConstants.STROKE_INVITE_PREFIX, entry.getKey(), tripid);
        }
        //删除司机的邀请 hset
        redisHelper.delKey(HitchConstants.STROKE_INVITE_PREFIX, tripid);

        //删除司机距离数据
        redisHelper.delKey(HitchConstants.STROKE_GEO_DISTANCE_PREFIX, tripid);
    ...
    public ResponseVO<StrokeVO> inviteList(String tripid) {
        Map<String, String> tripMap = redisHelper.getHashByMap(HitchConstants.STROKE_INVITE_PREFIX, tripid);
    ...
    public ResponseVO<StrokeVO> inviteAccept(StrokeVO strokeVO) {
        ...
        String inviterTripId = strokeVO.getInviterTripId();
        //判断司机是否已经发车
        boolean isDepart = redisHelper.exists(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId);
        if (!isDepart) {
        ...
        // 修改邀请状态
        // 0 = 未确认， 1 = 已确认 ， 2= 已拒绝
        redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviteeTripId, inviterTripId, String.valueOf(state.getCode()));
        redisHelper.addHash(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId, inviteeTripId, String.valueOf(state.getCode()));
    ...
    public List<StrokeVO> getZsetResulets(StrokePO strokePO) {
        List<ZsetResultBO> zsetResultBOList = redisHelper.getZsetSortValues(HitchConstants.STROKE_GEO_ZSET_PREFIX, strokePO.getId());
        List<StrokeVO> strokeVOS = new ArrayList<>();
        for (ZsetResultBO zsetResultBO : zsetResultBOList) {
    ...
    private void addOrder(StrokePO inviter, StrokePO invitee) {
        ...
        orderPO.setCreatedBy(invitee.getCreatedBy()); //创建人
        ...
        // 起点坐标，例如String start = "39.915,116.404";
        String start = invitee.getStartGeoLat() + "," + invitee.getStartGeoLng();
        // 终点坐标，例如String end = "40.05,116.27";
        ...
        //批量算路服务
        if (resultBO!=null) {
            orderPO.setDistance(resultBO.getDistance().getValue());//距离 单位米
            orderPO.setEstimatedTime(resultBO.getDuration().getValue());//预计时长 单位秒
            orderPO.setCost(valuation.calculation((float) orderPO.getDistance() /1000)); //价格 单位元
        }
    ...
    private int getSurplusSeats(String inviterTripId) {
        ...
        Map<String, String> driverMap = redisHelper.getHashByMap(HitchConstants.STROKE_INVITE_PREFIX, inviterTripId);
        //获取已确认的成员数量
        int member = 0;
    ...
    private void unbindStroke(StrokeVO strokeVO) {
        ...
        if (strokeVO.getRole().equals(0)) {
            ...
            //删除乘客端起点GEO信息
            redisHelper.delGEO(HitchConstants.STROKE_PASSENGER_GEO_START, "", unbindId);
            //删除乘客端终点点GEO信息
            redisHelper.delGEO(HitchConstants.STROKE_PASSENGER_GEO_END, "", unbindId);
        } else if (strokeVO.getRole().equals(1)) {
            unbindId = strokeVO.getInviterTripId();
            //删除司机端起点GEO信息
            redisHelper.delGEO(HitchConstants.STROKE_DRIVER_GEO_START, "", unbindId);
            //删除司机端终点点GEO信息
            redisHelper.delGEO(HitchConstants.STROKE_DRIVER_GEO_END, "", unbindId);
        }

        //取乘客的zset
        //通过乘客获取司机的
        List<ZsetResultBO> zsetResultBOList = redisHelper.getZsetSortValues(HitchConstants.STROKE_GEO_ZSET_PREFIX, unbindId);
        for (ZsetResultBO zsetResultBO : zsetResultBOList) {
            //删除司机的zset里所有当前乘客的id，包含确认同行的那个司机
            redisHelper.delZsetByKey(HitchConstants.STROKE_GEO_ZSET_PREFIX, zsetResultBO.getValue(), unbindId);
        }
        //删除乘客端zset
        redisHelper.delKey(HitchConstants.STROKE_GEO_ZSET_PREFIX, unbindId);

        //取乘客的邀请hset
        Map<String, String> driverMap = redisHelper.getHashByMap(HitchConstants.STROKE_INVITE_PREFIX, unbindId);
        ...
        for (Map.Entry<String, String> entry : driverMap.entrySet()) {
            ...
            //删除司机的hash关联关系
            redisHelper.delHash(HitchConstants.STROKE_INVITE_PREFIX, entry.getKey(), unbindId);
        }
        //删除乘客邀请hset
        redisHelper.delKey(HitchConstants.STROKE_INVITE_PREFIX, unbindId);

        //删除乘客距离hset
        redisHelper.delKey(HitchConstants.STROKE_GEO_DISTANCE_PREFIX, unbindId);
    }
    ...
    private StrokeVO renderStrokeVO(StrokeVO strokeVO) {
        //渲染距离数据
        String distanceStr = redisHelper.getHash(HitchConstants.STROKE_GEO_DISTANCE_PREFIX, strokeVO.getInviterTripId(), strokeVO.getInviteeTripId());
        //设置起点距离
    ...
    private Collection<HitchGeoBO> initGeoData(StrokePO strokePO) {
        ...
        for (HitchGeoBO hitchGeoBO : hitchGeoBOList) {
            //所有行程记录我
            redisHelper.addZset(HitchConstants.STROKE_GEO_ZSET_PREFIX, hitchGeoBO.getTargetId(), strokePO.getId(), getScore(hitchGeoBO));
            redisHelper.addHash(HitchConstants.STROKE_GEO_DISTANCE_PREFIX, hitchGeoBO.getTargetId(), strokePO.getId(), getDistanceStr(hitchGeoBO));
            //我记录所有行程
            redisHelper.addZset(HitchConstants.STROKE_GEO_ZSET_PREFIX, strokePO.getId(), hitchGeoBO.getTargetId(), getScore(hitchGeoBO));
            redisHelper.addHash(HitchConstants.STROKE_GEO_DISTANCE_PREFIX, strokePO.getId(), hitchGeoBO.getTargetId(), getDistanceStr(hitchGeoBO));
        }
    ...
    private void publishGeoData(StrokePO strokePO) {
        ...
        String start_key = strokePO.getRole() == 0 ? HitchConstants.STROKE_PASSENGER_GEO_START : HitchConstants.STROKE_DRIVER_GEO_START;
        //乘客/司机结束key
        String end_key = strokePO.getRole() == 0 ? HitchConstants.STROKE_PASSENGER_GEO_END : HitchConstants.STROKE_DRIVER_GEO_END;
        //添加开始结束的GEO信息
    ...
    private Collection<HitchGeoBO> geoFilterMatch(StrokePO strokePO) {
        //乘客/司机开始key 乘客筛选司机 。司机筛选乘客
        String start_key = strokePO.getRole() == 0 ? HitchConstants.STROKE_DRIVER_GEO_START : HitchConstants.STROKE_PASSENGER_GEO_START;
        //乘客/司机结束key
        String end_key = strokePO.getRole() == 0 ? HitchConstants.STROKE_DRIVER_GEO_END : HitchConstants.STROKE_PASSENGER_GEO_END;
        //开始geo数据
    ...
    private float getScore(HitchGeoBO hitchGeoBO) {
        return ((float) (1 - (hitchGeoBO.getStartGeo().getDistance() * 0.5 + hitchGeoBO.getEndGeo().getDistance() * 0.5) / HitchConstants.STROKE_DIAMETER_RANGE)) * 100;
    }
