...
import com.syduck.hitchcommons.domain.bo.GeoBO;
import com.syduck.hitchcommons.domain.bo.ZsetResultBO;
...
public class RedisHelper {
    ...
    public Map<String, GeoBO> geoNearByXY(String prefix, String key, float lng, float lat) {
        ...
        Circle circle = new Circle(
                new Point(lng, lat),
                new Distance(HitchConstants.STROKE_DIAMETER_RANGE, Metrics.KILOMETERS)
        );
        //限制20条，可以根据实际情况调整
    ...
    public Map<String, GeoBO> geoNearByPlace(String prefix, String key, String location, boolean isStart) {
        String redisKey = getRedisKey(prefix, key);
        Distance distance = new Distance(HitchConstants.STROKE_DIAMETER_RANGE, Metrics.KILOMETERS);//params: 距离量, 距离单位
    ...
    public void delHash(String prefix, String key, String... hkeys) {
        String redisKey = getRedisKey(prefix, key);
        redisTemplate.opsForHash().delete(redisKey, (Object) hkeys);
    }
    ...
    public List<ZsetResultBO> getZsetSortValues(String prefix, String key) {
        String redisKey = getRedisKey(prefix, key);
