package com.syduck.hitchgateway.configuration;
...
public class TokenAuthGatewayFilterFactory extends AbstractGatewayFilterFactory<TokenAuthGatewayFilterFactory.PathConfig> {
    ...
    public boolean verifyPassPath(RequestPath requestPath, PathConfig pathConfig) {
        ...
        List<String> filterList = Arrays.stream(passPathArray).filter(path -> path.equals(requestPath.toString())).toList();
    ...
    public GatewayFilter apply(PathConfig config) {
        return (exchange, chain) -> {
            ...
            exchange.getRequest().mutate().headers(httpHeaders -> {
                httpHeaders.add(HitchConstants.HEADER_ACCOUNT_KEY, accountID);
            });
            // 认证通过放行
            return chain.filter(exchange);
        };
    ...
    private static String getSessionToken(ServerHttpRequest request) {
        String sessionToken = request.getHeaders().getFirst(HitchConstants.SESSION_TOKEN_KEY);
        if (StringUtils.isEmpty(sessionToken)) {
            sessionToken = request.getQueryParams().getFirst(HitchConstants.SESSION_TOKEN_KEY);
        }
        return sessionToken;
    }
